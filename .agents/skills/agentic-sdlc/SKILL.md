...
1. Select only nodes whose dependencies satisfy the graph and whose conditions apply.
...
3. Give each agent only approved upstream artifacts, the task-local scope, its tool boundary, and the required output contract.
...
7. Stop at `approval: required`. Continue only after `scripts/workflow.py approve` records the actor and decision.
...
- Prefer discarding an unmerged isolated worktree. Revert commits only when discarding is not possible. Use explicit compensation for non-Git side effects.
